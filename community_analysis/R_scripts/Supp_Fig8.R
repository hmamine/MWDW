...
#subset to rhizo. samples
	physeq.sub=subset_samples( physeq_B, Comp == "Rhizo" )
...
#filtering OTUs by occurance
	...
	DT.tax$ColPhylum <- ifelse(!DT.tax$Phylum %in% ColPhylum, "Other", ifelse(DT.tax$Phylum == "Bacteroidetes", "Bacteroidetes", ifelse(DT.tax$Phylum == "Firmicutes", "Firmicutes", ifelse (DT.tax$Phylum == "Actinobacteria", "Actinobacteria", "Proteobacteria"))))
#enrichment test
...
###Tur
	...
	grid.echo()
	TUR<-grid.grab()
#pdf("Supplementary_Fig_8.pdf", useDingbats=FALSE, paper="A4")
