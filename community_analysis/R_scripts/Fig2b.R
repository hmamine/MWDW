...
for( i in LIST ) 
	...
	#computing WUF distances to centroid
...
#preparing the plot
	panel_b=ggplot(DT_M, aes(x=variable, y=value,  color=grp))+
	geom_boxplot(width=1)+
	geom_point(size=1, color="black")+theme_bw()+ theme_new +
	scale_colour_manual(values=color_palette) + 
	xlab ("Genotype x Niche")+
	ylab("Weighted UniFrac distances to centroid")

	#pdf("Fig2b.pdf", useDingbats=FALSE, paper ="A4")
	gridExtra::grid.arrange(panel_b, nrow=2, ncol=2)
	#dev.off()

##########################################################
### figure was edited and merged with fig2 panels a and c#
##########################################################
