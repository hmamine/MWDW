...
for (i in All) {
	...
	p1= pp1 + geom_jitter (aes(color=ColPhylum, shape=ShpClass),size=1.5) +
	theme_bw() + theme_change +
	scale_color_manual(values=Palette.phylum)+
	scale_shape_manual(values=Palette.shape.1)+
	geom_line(aes(x=log(p), y=freq.pred))+
	geom_line(aes(x=log(p), y=pred.lwr),linetype="longdash")+
	geom_line(aes(x=log(p), y=pred.upr),linetype="longdash")
	p[[i[[1]]]]<-p1
	print("######################################################################")
}
	...
	ppRM<-ggplot(sncm.stat, aes(x=Rsqr, y=m, color=Richness, shape=comp))
	p2=ppRM+geom_point(size=5)+ 
	scale_color_gradient2(low="darkcyan", mid="darkorange", high="darkviolet", midpoint=mid)+
	theme_bw() + theme_change + scale_shape_manual(values=Palette.shape.2)
#	pdf("Supplementary_Fig_15.pdf", paper="A4", useDingbats=FALSE)
	print(p2)
#	dev.off()
