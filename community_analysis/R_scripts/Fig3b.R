...
#	print( paste0("subset ", i, " samples" ))
	SUB=c( i )
	physeq_SUB=subset_samples( physeq_F, Comp %in% SUB )
...
#write.table(Bnull.out, "bnullout_fungi.txt", quote=FALSE, sep="\t")

# upload BC null diviations of fungi 
	...
	DT=DT.sd[DT.F]
	DT<-DT[,-c("BCF")]
	DT.m<-melt(DT, measure.vars =c("NullF"))
	DT.m$variable<-as.factor(DT.m$variable)
	DT.m$VAR<-paste0(DT.m$Concat,DT.m$variable)
	DT.m$kingdom<-ifelse(DT.m$variable=="NullF","F","NA")

# preparing the plot
	pp2 <- ggplot(data=DT.m[DT.m$kingdom=="F",], aes(x=VAR, y=as.numeric(value)))
	panel_b=pp2+geom_boxplot( aes(color=Geno)) + geom_point(size=1) +
	scale_y_continuous(name="Abundance Null Deviation")+theme_bw(base_size=10)+
	scale_color_manual(values=color_palette) + theme_new 
...
#testing for significance
	DT.leaf<-p2$data[p2$data$Comp == "Leaf",]
	kruskal.test(data=DT.leaf, value ~ as.factor(VAR))

	DT.root<-p2$data[p2$data$Comp == "Root",]
	kruskal.test(data=DT.root, value ~ as.factor(VAR))
	posthoc.kruskal.conover.test(data=DT.root, value ~ as.factor(VAR), method="BH")

	DT.rhizo<-p2$data[p2$data$Comp == "Rhizo",]
	kruskal.test(data=DT.rhizo, value ~ as.factor(VAR))

##########################################################
###### figure was edited and merged with fig 3a    #######
##########################################################
